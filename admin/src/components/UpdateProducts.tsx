...
const UpdateProduct: React.FC<UpdateProductProps> = () => {
  ...
  const handleOptionChange = (e: ChangeEvent<HTMLInputElement>, variantIndex: number, optionIndex: number, field: keyof OptionValue) => {
    const updatedVariants = [...variants];
    (updatedVariants[variantIndex].optionValues[optionIndex][field] as string[]) = [e.target.value];
    setVariants(updatedVariants);
    ...
    const fetchProduct = async () => {
      try {
        ...
        if (product.productVariants) {
          const variantsData = product.productVariants.map((variant: {
              key: string;
              value: string[] | null;
      ...
      <tr key={index}>
        ...
        <td>
          <input
            ...
            onChange={e => handleVariantChange(e, index, 'options')}
          />
        </td>
        <td>
          <input
  ...
  {variant.optionValues.map((option, optionIndex) => (
    <div key={optionIndex} className="option-value">
      <input
        type="text"
        value={option.name}
        onChange={e => handleOptionChange(e, index, optionIndex, 'name')}
